...
async def test_process_report_sets_completed(
    test_engine: AsyncEngine, test_report: Report
) -> None:
    # Patch asyncio.sleep to avoid 2-8s delay in tests.
    with patch("app.tasks.asyncio.sleep"), patch(
        "app.tasks._make_engine", return_value=test_engine
    ):
...
async def test_fail_report_sets_failed(
    test_engine: AsyncEngine, test_report: Report
) -> None:
