...
async def test_get_report_not_found_returns_404(client: AsyncClient) -> None:
    r = await client.get("/reports/00000000-0000-0000-0000-000000000000", headers=USER_HEADER)
...
async def test_get_report_wrong_user_returns_404(client: AsyncClient) -> None:
    ...
    r = await client.get(f"/reports/{report_id}", headers={"X-User-Id": "other-user"})
    assert r.status_code == 404
...
async def test_create_report_missing_auth_returns_401(client: AsyncClient) -> None:
    r = await client.post("/reports")
    assert r.status_code == 401


async def test_list_reports_missing_auth_returns_401(client: AsyncClient) -> None:
